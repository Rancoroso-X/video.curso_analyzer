...
import time
from rich.progress import Progress, BarColumn, TextColumn, TimeRemainingColumn, TimeElapsedColumn, SpinnerColumn # Importar Progress aqui
...
def exibir_resumo_breve(modulos: dict, curso_nome: str):
    ...
    dur_total = sum(extrair_duracao(aula) for aulas in modulos.values() for aula in aulas)
    print(f"\n📦 Curso: {curso_nome}")
    print(f"📁 Módulos: {len(modulos)}")
    print(f"🎞️ Aulas: {total_aulas}")
    print(f"⏱️ Duração total: {segundos_para_hms(dur_total)}")
    print(f"🕒 Estimativa de tempo de transcrição: {segundos_para_hms(dur_total * 1.2)} (aprox. para o modelo base, Small/Faster Whisper será mais rápido).")
...
def escolher_modelo_whisper() -> str:
    ...
    modelo = input("Digite o modelo desejado (ou deixe em branco para usar 'small'): ").strip()
    return modelo if modelo else "small"

def main():
    print("🎬 ANALISADOR DE CURSOS 📚\n")
    caminho = input("📁 Digite o caminho da pasta com os cursos: ").strip().strip("'\"")
    ...
    while True:
        ...
        if escolha == "1":
            ...
            deletar_audio = input("🧹 Apagar áudios após gerar legendas/transcrição? (s/n): ").strip().lower() == 's'
            ...
            with Progress(
                ...
                transcrever_videos(resultado, modelo=modelo, tipo_audio=tipo_audio, deletar_audio=deletar_audio, progress=progress)
            
            fim = time.time()

            duracao_total = fim - inicio
            print(f"⏱️ Tempo total de execução: {segundos_para_hms(duracao_total)}")
